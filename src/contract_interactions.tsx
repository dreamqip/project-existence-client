...
const ORGANISATION_ABI = [
  ...
  'function register(uint256 id) public view returns (address)',
...
const REGISTER_ABI = [
  ...
  'function metadata() public view returns (string)',
  'function (bytes32 id) public records returns (Record)',
];
...
type RegisterContract = ethers.Contract & {
  ...
  metadata: () => Promise<string>;
  records: (documentHash: DataHexString) => Promise<any>;
};
export type {
...
export async function updateProvider(chain: 'mainnet' | 'testnet' | 'fakenet') {
  ...
  provider =
    rawProvider == null
      ? null
      : new ethers.BrowserProvider(rawProvider as any, chainID);
  if (provider != null) {
